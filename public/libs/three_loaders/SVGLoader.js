        ...
        parse(text) {
            ...
            function parseNode(node, style) {
                ...
                switch (node.nodeName) {
                    ...
                    case "use":
                        ...
                        const usedNode =
                            node.viewportElement.getElementById(usedNodeId);
            ...
            function parseArcCommand(
                ...
                const y1p =
                    ...
                    Math.cos(x_axis_rotation) * dy2; // Compute (cx', cy')
            ...
            function parseStyle(node, style) {
                ...
                function addStyle(svgName, jsName, adjustFunction) {
                    ...
                    if (node.hasAttribute(svgName))
                        style[jsName] = adjustFunction(
                            node.getAttribute(svgName)
                        );
                    if (stylesheetStyles[svgName])
                        style[jsName] = adjustFunction(
                            stylesheetStyles[svgName]
                        );
                ...
                function newNumber() {
                    if (number !== "") {
                        if (exponent === "") result.push(Number(number));
                            ...
                            result.push(
                                Number(number) * Math.pow(10, Number(exponent))
                            );
                    }
            ...
            function parseFloatWithUnits(string) {
                ...
                if (theUnit === "px" && scope.defaultUnit !== "px") {
                    ...
                    scale =
                        unitConversion["in"][scope.defaultUnit] /
                        scope.defaultDPI;
                } else {
                    ...
                    if (scale < 0) {
                        ...
                        scale =
                            unitConversion[theUnit]["in"] * scope.defaultDPI;
                    }
                }
            ...
            function parseNodeTransform(node) {
                ...
                if (node.hasAttribute("transform")) {
                    const transformsTexts = node
                        .getAttribute("transform")
                        .split(")");

                    for (
                        let tIndex = transformsTexts.length - 1;
                        tIndex >= 0;
                        tIndex--
                    ) {
                        ...
                        if (openParPos > 0 && openParPos < closeParPos) {
                            const transformType = transformText.slice(
                                0,
                                openParPos
                            );
            ...
            function findEdgeIntersection(a0, a1, b0, b1) {
                ...
                if (
                    ...
                    for (let i = 0; i < 2; i++) {
                        ...
                        if (
                            classifyResult.loc ==
                            IntersectionLocationType.ORIGIN
                        ) {
                            ...
                            const x = +(
                                x1 +
                                classifyResult.t * (x2 - x1)
                            ).toPrecision(10);
                            const y = +(
                                y1 +
                                classifyResult.t * (y2 - y1)
                            ).toPrecision(10);
                            return {
                                x: x,
                                y: y,
                ...
                while (
                    ...
                    if (
                        ...
                        stack[stack.length - 1] ===
                            otherIntersections[i].identifier
                    ) {
                        stack.pop();
                    } else {
