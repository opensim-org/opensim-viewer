(function () {
    ...
    class FBXTreeParser {
        ...
        parseImages() {
            ...
            if ("Video" in fbxTree.Objects) {
                ...
                for (const nodeID in videoNodes) {
                    ...
                    images[id] =
                        videoNode.RelativeFilename || videoNode.Filename; // raw image data is in videoNode.Content
        ...
        parseMaterial(materialNode, textureMap) {
            ...
            switch (type.toLowerCase()) {
                ...
                default:
                    console.warn(
                        'THREE.FBXLoader: unknown material type "%s". Defaulting to THREE.MeshPhongMaterial.',
                        type
                    );
                    material = new THREE.MeshPhongMaterial();
                    break;
        ...
        parseParameters(materialNode, textureMap, ID) {
            ...
            connections.get(ID).children.forEach(function (child) {
                ...
                switch (type) {
                    case "Bump":
                        parameters.bumpMap = scope.getTexture(
                            textureMap,
                            child.ID
                        );
                        break;
                    ...
                    case "Maya|TEX_ao_map":
                        parameters.aoMap = scope.getTexture(
                            textureMap,
                            child.ID
                        );
                        break;
                    ...
                    case "EmissiveColor":
                        ...
                        if (parameters.emissiveMap !== undefined) {
                            parameters.emissiveMap.encoding =
                                THREE.sRGBEncoding;
                        }
                    ...
                    case "ReflectionColor":
                        parameters.envMap = scope.getTexture(
                            textureMap,
                            child.ID
                        );
                    ...
                    case "SpecularColor":
                        ...
                        if (parameters.specularMap !== undefined) {
                            parameters.specularMap.encoding =
                                THREE.sRGBEncoding;
                        }
            ...
            if (lightAttribute === undefined) {
                ...
                let intensity =
                    lightAttribute.Intensity === undefined
                        ? 1
                        : lightAttribute.Intensity.value / 100; // light disabled
    ...
    class GeometryParser {
            ...
            if (geoNode.LayerElementUV) {
                ...
                while (geoNode.LayerElementUV[i]) {
                    if (geoNode.LayerElementUV[i].UV) {
                        geoInfo.uv.push(
                            this.parseUVs(geoNode.LayerElementUV[i])
                        );
                    }
        ...
        genFace(
            ...
            for (let i = 2; i < faceLength; i++) {
                ...
                buffers.vertex.push(
                    geoInfo.vertexPositions[
                        facePositionIndexes[(i - 1) * 3 + 1]
                    ]
                );
                buffers.vertex.push(
                    geoInfo.vertexPositions[
                        facePositionIndexes[(i - 1) * 3 + 2]
                    ]
                );
                buffers.vertex.push(
        ...
        parseNurbsGeometry(geoNode) {
            ...
            const curve = new THREE.NURBSCurve(
                degree,
                knots,
                controlPoints,
                startKnot,
                            ...
                            if (
                                ...
                                if (layerCurveNodes[i] === undefined) {
                                    ...
                                    if (modelID !== undefined) {
                                        ...
                                        if (!node.transform)
                                            node.transform =
                                                new THREE.Matrix4(); // if the animated model is pre rotated, we'll have to apply the pre rotations to every
                                ...
                                if (layerCurveNodes[i])
                                    layerCurveNodes[i][curveNode.attr] =
                                        curveNode;
                                ...
                                if (layerCurveNodes[i] === undefined) {
                                    ...
                                    const morpherID =
                                        connections.get(deformerID).parents[0]
                                            .ID;
                                    const geoID =
                                        connections.get(morpherID).parents[0]
                                            .ID; // assuming geometry is not used in more than one model

                                    const modelID =
                                        connections.get(geoID).parents[0].ID;
                                    const rawModel =
                                        fbxTree.Objects.Model[modelID];
                                    const node = {
                                        modelName: rawModel.attrName
                                            ? THREE.PropertyBinding.sanitizeNodeName(
                                                  rawModel.attrName
    ...
    class TextParser {
        ...
        parseNodeProperty(line, property, contentLine) {
            ...
            let propValue = property[2]
                .replace(/^"/, "")
                .replace(/"$/, "")
                .trim(); // for special case: base64 image data follows "Content: ," line
            //	Content: ,
            //	 "/9j/4RDaRXhpZgAATU0A..."
    ...
    function generateTransform(transformData) {
        ...
        if (transformData.scale)
            lScalingM.scale(tempVec.fromArray(transformData.scale)); // Pivots and offsets
        ...
        const lLRM = lPreRotationM
            .clone()
            .multiply(lRotationM)
            .multiply(lPostRotationM); // Global Rotation
        ...
        if (inheritType === 0) {
            ...
            const lParentLSM = new THREE.Matrix4().scale(
        ...
        const lLocalTWithAllPivotAndOffsetInfo =
            new THREE.Matrix4().copyPosition(lTransform);
    ...
    function getEulerOrder(order) {
        ...
        if (order === 6) {
            console.warn(
                "THREE.FBXLoader: unsupported THREE.Euler Order: Spherical XYZ. Animations and rotations may be incorrect."
            );
            return enums[0];
        }
